...
class SlackPlugin {
    constructor(script, events) {
        ...
        const nameTag = (global.artillery.testInfo?.tags || []).find((t) => t.name === 'name');
        if (nameTag) {
            this.testName = truncateForSlackHeader(nameTag.value);
        }
        if (this.script.config.plugins.ensure &&
    ...
    getErrors(report) {
        const errorList = [];
        for (const [key, value] of Object.entries(report.counters).filter(([key]) => key.startsWith('errors.'))) {
    ...
    assembleSlackPayload(report, ensureChecks) {
        ...
        const duration = report.lastMetricAt != null && report.firstMetricAt != null
            ? report.lastMetricAt - report.firstMetricAt
            : undefined;
        // Ensure exitCode has a value before using it
        const exitCode = this.exitCode !== undefined && this.exitCode !== null ? this.exitCode : 0;
        const testLabel = this.testName ? ` (${this.testName})` : '';
        const headerText = exitCode === 0
        ...
        if (this.ensureChecks) {
            ...
            if (this.ensureChecks.total > 10) {
                ...
                const formattedChecks = this.ensureChecks.checkList.map((check) => `${check.passed ? '🟢' : '🔴'} ${check.text}${check.optional ? ' (optional)' : ''}`);
