...
import * as EXPECTATIONS from "./lib/expectations.js";
import * as FORMATTERS from "./lib/formatters.js";
const debug = createDebug('plugin:expect');
export { ExpectationsPlugin as Plugin, EXPECTATIONS as expectations, FORMATTERS as formatters };
function ExpectationsPlugin(script, events) {
    ...
    script.config.processor.expectationsPluginSetExpectOptions = (userContext, _events, done) => {
...
function expectationsPluginOnError(scenarioErr, _requestParams, userContext, _events, done) {
...
function expectationsPluginCheckExpectations(req, res, userContext, events, done) {
    ...
    _.each(expectations, (ex) => {
        ...
        if (EXPECTATIONS[checker]) {
            result = EXPECTATIONS[checker].call(this, ex, body, req, res, userContext);
            results.push(result);
        }
        else {
    ...
    FORMATTERS[formatterName].call(this, requestExpectations, req, res, userContext);
    ...
    if (userContext.expectationsPlugin.reportFailuresAsErrors) {
        const filteredRequestName = userContext.expectationsPlugin.useOnlyRequestNames && req.name
            ? req.name
            : req.url;
        return done(new FailedExpectationError(`Failed expectations for request ${filteredRequestName}`));
    }
    return done();
}
