...
export { prettyPrint as pretty, jsonPrint as json, prettyError, silent };
...
function prettyPrint(requestExpectations, req, res, userContext) {
    ...
    requestExpectations.results.forEach((result) => {
        console.log(`  ${result.ok ? chalk.green('ok') : chalk.red('not ok')} ${result.type} ${result.got} `);
        if (!result.ok) {
...
function printExchangeContext(req, res, userContext) {
    ...
    Object.keys(userContext.vars)
        .filter((varName) => varName !== '$processEnvironment' && varName !== '$env')
        .forEach((varName) => {
