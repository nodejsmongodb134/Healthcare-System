...
const flattenName = (moduleName, functionName) => `${moduleName}${functionName[0].toUpperCase()}${functionName.slice(1)}`;
...
function ArtilleryPluginFakeData(script, events) {
    this.script = script;
    this.events = events;
    const pluginConfig = script.config['fake-data'] || script.config.plugins['fake-data'];
        ...
        for (const [aliasName, funcName] of Object.entries(DEPRECATED_ALIASES)) {
            context.funcs[`$${aliasName}`] = () => {
                if (!warnedAliases.has(aliasName)) {
                    ...
                    console.warn(`[fake-data] $${aliasName} is deprecated and will be removed in a future release. Use $${funcName} instead.`);
                }
...
export { ArtilleryPluginFakeData as Plugin, getFakerFunctions, getDeprecatedAliases };
