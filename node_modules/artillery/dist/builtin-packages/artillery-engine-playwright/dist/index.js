...
class PlaywrightEngine {
    ...
    createScenario(spec, events) {
        ...
        return async function scenario(initialContext, cb) {
            ...
            if (self.vusRecording < self.MAX_CONCURRENT_RECORDINGS &&
            ...
            try {
                ...
                await context.addInitScript(() => {
                    ['onLCP', 'onFCP', 'onCLS', 'onTTFB', 'onINP'].forEach((hook) => {
                        globalThis.webVitals[hook]((metric) => {
                ...
                page.on('domcontentloaded', async (page) => {
                    ...
                    try {
                        ...
                        const startToInteractive = performanceTiming.domInteractive -
                            performanceTiming.navigationStart;
                        events.emit('counter', self.processor.$rewriteMetricName('browser.page.domcontentloaded', 'counter'), 1);
                        events.emit('counter', self.processor.$rewriteMetricName(`browser.page.domcontentloaded.${getName(page.url())}`, 'counter'), 1);
                        events.emit('histogram', self.processor.$rewriteMetricName('browser.page.dominteractive', 'histogram'), startToInteractive);
                        events.emit('histogram', self.processor.$rewriteMetricName(`browser.page.dominteractive.${getName(page.url())}`, 'histogram'), startToInteractive, { url: page.url(), vuId: initialContext.vars.$uuid });
                    }
                    catch (_err) { }
                });
                    ...
                    if (msg.type() === 'trace') {
                        ...
                        try {
                            ...
                            if (url.startsWith(self.target) || self.showAllPageMetrics) {
                                events.emit('histogram', self.processor.$rewriteMetricName(`browser.page.${name}.${getName(url)}`, 'histogram'), value, {
                ...
                page.on('load', async (page) => {
                    ...
                    try {
                        ...
                        const { usedJSHeapSize } = JSON.parse(await page.evaluate(() => JSON.stringify({
                            usedJSHeapSize: window.performance.memory
                                .usedJSHeapSize
                        })));
                        events.emit('histogram', self.processor.$rewriteMetricName('browser.memory_used_mb', 'histogram'), usedJSHeapSize / 1000 / 1000, { url: page.url(), vuId: initialContext.vars.$uuid });
                    }
                    catch (_err) { }
                });
                ...
                const fn = self.processor[spec.testFunction] ||
                    self.processor[spec.flowFunction] ||
                    spec.testFunction;
                if (!fn) {
                    console.error('Playwright test function not found:', fn);
                    return cb(new Error('Playwright test function not found'), initialContext);
                }
                const test = { step };
                let traceScenario;
                if (self.tracing) {
                ...
                if (initialContext.vars.isRecording) {
                    if (Date.now() - self.lastTraceRecordedTime >
