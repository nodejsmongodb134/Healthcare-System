...
import { getHashedVarToValueMap, hashString, replaceMetricsWithHashes } from "./utils.js";
        ...
        global.artillery.ext({
            ...
            method: async (data) => {
                ...
                checkTests
                    ...
                    .forEach((check) => {
                    if (check.result !== 1) {
                        global.artillery.log(`${chalk.red('fail')}: ${check.original}${check.strict ? '' : ' (optional)'}`);
                        if (check.strict) {
        ...
        if (Array.isArray(checks.conditions)) {
            checks.conditions.forEach((o) => {
                if (typeof o === 'object') {
                    ...
                    const hashedExpression = replaceMetricsWithHashes(Object.keys(vars), expression);
                    let f = () => { };
                    try {
                        f = filtrex(hashedExpression);
                    }
