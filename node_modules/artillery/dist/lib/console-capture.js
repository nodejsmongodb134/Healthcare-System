...
function setupConsoleCapture() {
    ...
    let currentSize = 0;
    let sendFromIndex = 0;
    const MAX_RETAINED_LOG_SIZE_MB = Number(process.env.MAX_RETAINED_LOG_SIZE_MB || '50');
    ...
    global.artillery.ext({
        ...
        method: async () => {
            ...
            if (!truncated && sendFromIndex < outputLines.length) {
                const ts = Date.now();
                global.artillery.globalEvents.emit('logLines', outputLines.slice(sendFromIndex, outputLines.length), ts, true);
                sendFromIndex = outputLines.length;
            }
        }
    });
