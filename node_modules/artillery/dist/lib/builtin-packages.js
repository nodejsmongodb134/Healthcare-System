...
export const BUILTIN_PACKAGES_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'builtin-packages');
...
export function resolveBuiltinPackage(name) {
    try {
...
// the namespace.
export async function loadBuiltinPackage(name) {
    const ns = await import(__rewriteRelativeImportExtension(pathToFileURL(resolveBuiltinPackage(name)).href));
