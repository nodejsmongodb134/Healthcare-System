...
class RunLambdaCommand extends Command {
    ...
    async run() {
        ...
        if (flags['security-group-ids']) {
            flags['platform-opt'].push(`security-group-ids=${flags['security-group-ids']}`);
        }
        if (flags['subnet-ids']) {
...
RunLambdaCommand.flags = {
    ...
    'security-group-ids': Flags.string({
        description: 'Comma-separated list of security group IDs to use for the Lambda function'
    }),
    'subnet-ids': Flags.string({
        description: 'Comma-separated list of subnet IDs to use for the Lambda function'
    }),
    'aws-tags': Flags.string({
        description: 'Comma-separated list of tags in key:value format to apply to AWS resources created for the test run (Lambda function and SQS queue), for example: --aws-tags "team:perf,cost-center:1234"'
    })
};
...
export default RunLambdaCommand;
