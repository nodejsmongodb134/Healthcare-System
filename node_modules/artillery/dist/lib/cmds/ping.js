...
import { engine_http as HttpEngine } from "../core/index.js";
    ...
    async run() {
        ...
        if (flags.form) {
            ...
            try {
                ...
                console.error(chalk.red('Could not parse value of --form as valid JSON, YAML or key-value string'));
                console.error(chalk.red(flags.form));
                console.error(chalk.red(parseErr.message));
                console.error('Example: send a form containing two fields:\n  --form "{name: tiki, type: pony}"');
                process.exit(1);
            }
        ...
        try {
            ...
            this.log(`HTTP/${context.vars.httpVersion} ${context.vars.statusCode < 400
            ...
            for (const headerName of Object.keys(context.vars.headers)) {
                this.log(`${headerName}: ${chalk.blue(context.vars.headers[headerName])}`);
            }
            ...
            this.log('\n\n', format(TEMPLATE, 
            // Phase durations:
            fmt(phases.dns || 0, 10), fmt(phases.tcp || 0, 14), fmt(phases.tls || 'N/A', 10), fmt(phases.firstByte, 15), fmt(phases.download, 13), 
            // Cumulative:
            fmt(phases.dns || 0), fmt(timings.connect - timings.socket || 0), fmt((timings.secureConnect || timings.connect) - timings.socket || 0), fmt(timings.response - timings.socket), fmt(timings.end - timings.socket)).replace(/\|/g, chalk.gray('|')));
            if (parseInt(context.vars.res.httpVersion, 10) > 1) {
                this.log(chalk.gray('(NOTE: DNS, TCP and SSL overhead is not reported for HTTP/2 yet)'));
            }
            const contentType = context.vars.headers?.['content-type'] || 'application/octet-stream';
            ...
            if (flags.showBody) {
                if (!flags.pretty) {
                    ...
                    if (isJSON) {
                        if (!isBodyValidJSON) {
                            this.log(chalk.red('Could not parse body as valid JSON for pretty-printing'));
                        }
                        else {
            ...
            if (checks.length > 0) {
                const { expectations } = await loadBuiltinPackage('artillery-plugin-expect');
                this.log(chalk.cyan('Expectations:\n'));
                const results = [];
                for (const ex of checks) {
                    const checker = Object.keys(ex)[0];
                    const result = expectations[checker].call(this, ex, body, context.vars.req, context.vars.res, {});
                    results.push(result);
                }
                ...
                results.forEach((result) => {
                    this.log(`*  ${result.ok ? chalk.green('ok') : chalk.red('not ok')} ${result.type} ${result.got} `);
                    if (!result.ok) {
...
PingCommand.flags = {
    ...
    h1: Flags.boolean({
        description: 'Force HTTP/1.1 (HTTP/2 is used by default when supported by the server)'
    }),
    expect: Flags.string({
        char: 'e',
        multiple: true,
