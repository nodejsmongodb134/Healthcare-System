...
RunACICommand.flags = {
    ...
    cpu: Flags.string({
        description: 'Number of CPU cores per worker (defaults to 4 CPUs). A number between 1-4.'
    }),
    memory: Flags.string({
        description: 'Memory in GB per worker (defaults to 8 GB). A number between 1-16.'
    }),
    'tenant-id': Flags.string({
        description: 'Azure tenant ID. May also be set via AZURE_TENANT_ID environment variable.'
    }),
    'subscription-id': Flags.string({
        description: 'Azure subscription ID. May also be set via AZURE_SUBSCRIPTION_ID environment variable.'
    }),
    'storage-account': Flags.string({
        description: 'Azure Blob Storage account name. May also be set via AZURE_STORAGE_ACCOUNT environment variable.'
    }),
    'blob-container': Flags.string({
        description: 'Azure Blob Storage container name. May also be set via AZURE_STORAGE_BLOB_CONTAINER environment variable.'
    }),
    'resource-group': Flags.string({
        description: 'Azure Resource Group name. May also be set via AZURE_RESOURCE_GROUP environment variable.'
    })
};
RunACICommand.args = {
    script: Args.string({
