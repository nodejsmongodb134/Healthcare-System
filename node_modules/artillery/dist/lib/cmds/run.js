...
class RunCommand extends Command {
    ...
    async run() {
        ...
        if (flags.platform === 'aws:ecs') {
            ...
            const RunFargateCommand = (await import("./run-fargate.js")).default;
...
RunCommand.flags = {
    ...
    'platform-opt': Flags.string({
        description: 'Set a platform-specific option, e.g. --platform-opt region=eu-west-1 for AWS Lambda',
        multiple: true
    }),
    count: Flags.string({
...
RunCommand.runCommandImplementation = async (flags, argv, args) => {
    ...
    if (tagResult.errors.length > 0) {
        console.log('WARNING: could not parse some tags:', tagResult.errors.join(', '));
    }
    ...
    var launcher = await createLauncher(runScript, runScript.config.payload, runnerOpts, launcherOpts);
    if (!launcher) {
    ...
    if (process.env.CUSTOM_REPORTERS) {
        ...
        for (const name of customReporterNames) {
            ...
            const ns = await import(__rewriteRelativeImportExtension(pathToFileURL(reporterPath).href));
            const createReporter = ns.default ?? ns;
            const reporter = createReporter(activeLauncher.events, flags);
            reporters.push(reporter);
        }
    }
    activeLauncher.events.on('phaseStarted', (_phase) => { });
    activeLauncher.events.on('stats', (stats) => {
    ...
    activeLauncher.events.on('done', async (stats) => {
        let report;
        if (artillery.runtimeOptions.legacyReporting) {
    ...
    global.artillery.ext({
        ...
        method: async (event) => {
            try {
                const duration = Math.round((event.report?.lastMetricAt - event.report?.firstMetricAt) / 1000);
        ...
        await (async () => {
            ...
            if (global.artillery.hasTypescriptProcessor &&
                ...
                try {
                    ...
                    console.log(`WARNING: Failed to remove typescript bundled file: ${global.artillery.hasTypescriptProcessor}`);
                    console.log(err);
                }
                try {
...
async function sendTelemetry(script, flags, extraProps) {
    ...
    try {
        ...
        if (script.config.plugins['publish-metrics']) {
            ...
            properties.officialMonitoringReporters = script.config.plugins['publish-metrics']
                .map((reporter) => {
