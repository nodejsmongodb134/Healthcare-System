...
import { getCloudHttpClient, getGot } from "./http-client.js";
...
class ArtilleryCloudPlugin {
    constructor(_script, _events, { flags }) {
        ...
        const enabledInCloudWorker = typeof process.env.WORKER_ID !== 'undefined' &&
        ...
        if (isInteractiveUse) {
            ...
            global.artillery.globalEvents.on('done', async (report) => {
                debug('done');
                debug('testrun:aggregatereport: payload size:', JSON.stringify(report).length);
        ...
        global.artillery.ext({
            ...
            method: async (opts) => {
                ...
                if (isInteractiveUse) {
                    ...
                    await awaitOnEE(global.artillery.globalEvents, 'logLines', 200, 1 * 1000 //wait at most 1 second for a final log lines event emitter to be fired
                    );
                }
    ...
    async init() {
        ...
        console.log(`Run URL: ${this.baseUrl}/${this.orgId}/load-tests/${global.artillery.testRunId}`);
        this.user = {
            id: body.id,
            email: body.email
        };
        const outputDir = process.env.PLAYWRIGHT_TRACING_OUTPUT_DIR ||
    ...
    async _uploadAsset(localFilename) {
        ...
        try {
            ...
            try {
                ...
                if (res.statusCode !== 200) {
                    console.error(`Could not get upload URL for Playwright trace recording: ${filename} (HTTP ${res.statusCode})`);
                ...
                const err = caughtErr;
                console.error('Could not get upload URL for Playwright trace recording:', filename, err.code || err.name, err.message);
            ...
            try {
                ...
                const error = caughtErr;
                console.error('Failed to upload Playwright trace recording:', filename, error.code || error.name, error.message);
                if (error.response) {
    ...
    async waitOnUnprocessedLogs(maxWaitTime) {
        let waitedTime = 0;
        while ((this.unprocessedLogsCounter > 0 || this.uploading > 0) &&
    ...
    setGetStatusInterval() {
        const interval = setInterval(async () => {
            ...
            console.log(`WARNING: Artillery Cloud user ${res.cancelledBy} requested to stop the test. Stopping test run - this may take a few seconds.`);
    ...
    async _event(eventName, eventPayload) {
        ...
        try {
            ...
            if (res.statusCode !== 200) {
                if (res.statusCode === 401) {
                    console.log('Error: API key is invalid. Could not send test data to Artillery Cloud.');
                }
                else {
