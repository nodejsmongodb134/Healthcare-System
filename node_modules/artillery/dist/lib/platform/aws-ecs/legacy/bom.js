...
import * as builtinPlugins from "./plugins.js";
const debug = createDebug('bom');
// require.resolve with explicit paths (resolving user test dependencies
// relative to the test script dir, not this file)
const require = createRequire(import.meta.url);
const BUILTIN_PLUGINS = builtinPlugins.getAllPluginNames();
const BUILTIN_ENGINES = builtinPlugins.getOfficialEngines();
import Table from 'cli-table3';
import prepareTestExecutionPlan from "../../../util/prepare-test-execution-plan.js";
import { parseScript, readScript, resolveConfigTemplates } from "../../../util.js";
    ...
    A.waterfall([
        ...
        async (scriptPath) => {
            ...
            if (scriptPath.toLowerCase().endsWith('.ts')) {
                scriptData = await prepareTestExecutionPlan([scriptPath], 
        ...
        if (context.localFilePaths.length === 1) {
            prefix = context.localFilePaths[0].substring(0, context.localFilePaths[0].length -
        ...
        const dependencyFiles = packageDescriptionFiles.map((s) => path.join(prefix, s));
        ...
        const files = context.localFilePaths.map((p) => {
            return {
                ...
                noPrefixPosix: _convertToPosixPath(p).substring(prefix.length, p.length)
            };
        });
        ...
        if (pkgPath) {
            ...
            const pkgDeps = [].concat(Object.keys(pkg.dependencies || {}), Object.keys(pkg.devDependencies || {}));
        ...
        const modules = _.uniq(context.npmModules).filter((m) => m !== 'artillery' &&
            m !== 'playwright' &&
...
function applyScriptChanges(context, next) {
    ...
    context.opts.scriptData, context.opts.flags, context.opts.absoluteScriptPath, context.opts.scenarioPath).then((resolvedConfig) => {
        context.opts.scriptData = resolvedConfig;
        return next(null, context);
    });
}
function getPlugins(context, next) {
    const environmentPlugins = _.reduce(_.get(context, 'opts.scriptData.config.environments', {}), function getEnvironmentPlugins(acc, envSpec, _envName) {
        acc = acc.concat(Object.keys(envSpec.plugins || []));
        return acc;
    }, []);
    const pluginNames = Object.keys(_.get(context, 'opts.scriptData.config.plugins', {})).concat(environmentPlugins);
    const pluginPackages = _.uniq(pluginNames
...
function getCustomEngines(context, next) {
    const environmentEngines = _.reduce(_.get(context, 'opts.scriptData.config.environments', {}), function getEnvironmentEngines(acc, envSpec, _envName) {
        acc = acc.concat(Object.keys(envSpec.engines || []));
        return acc;
    }, []);
    const engineNames = Object.keys(_.get(context, 'opts.scriptData.config.engines', {})).concat(environmentEngines);
    const enginePackages = _.uniq(engineNames
...
async function getCustomJsDependencies(context) {
    ...
    context.localFilePaths = _.uniq(context.localFilePaths.concat(traceResult.localFiles));
    context.npmModules = context.npmModules.concat(traceResult.npmPackages);
    for (const pkg of traceResult.npmPackages) {
        if (context.moduleVersions[pkg])
            continue;
        const version = resolvePackageVersion(pkg, resolveRoot);
        if (version)
            context.moduleVersions[pkg] = version;
    }
    context.unresolvedImports = context.unresolvedImports.concat(traceResult.unresolved);
...
function ensureEsbuildInitialized(esbuildMod) {
...
function getVariableDataFiles(context, next) {
    ...
    context.localFilePaths = context.localFilePaths.concat(resolvePayloadPaths(context.opts.scriptData.config));
    context.opts.scriptData.config.environments =
        context.opts.scriptData.config.environments || {};
    Object.keys(context.opts.scriptData.config.environments).forEach((envName) => {
        const envSpec = context.opts.scriptData.config.environments[envName];
        context.localFilePaths = context.localFilePaths.concat(resolvePayloadPaths(envSpec));
    });
    return next(null, context);
}
    ...
    if (context.opts.scriptData.config?.plugins?.['http-file-uploads']) {
        ...
        if (context.opts.scriptData.config.plugins['http-file-uploads'].filePaths) {
            ...
            const baseDir = context.opts.scenarioPath
                ? path.dirname(context.opts.scenarioPath)
                : path.dirname(context.opts.absoluteScriptPath);
            const absPaths = context.opts.scriptData.config.plugins['http-file-uploads'].filePaths.map((p) => {
...
function expandDirectories(context, next) {
    ...
    dirs.forEach((d) => {
        ...
        debug(entries);
        context.localFilePaths = context.localFilePaths.concat(entries.map((e) => {
...
function prettyPrint(manifest) {
    ...
    for (const m of manifest.modules) {
        ...
        const notes = [];
        if (manifest.pkgDeps.indexOf(m) === -1)
            notes.push('not in package.json');
        if (version)
            notes.push(`v${version}`);
        t.push([m, 'package', notes.join(' · ')]);
    }
    artillery.log(t.toString());
    const unresolvedExternals = (manifest.externals || []).filter((e) => e.reason !== 'not-in-package-json');
...
// NOTE: getCustomJsDependencies was previously consumed by
// create-bom/create-bom.js without being exported (silently undefined
// under CJS); ESM named imports are validated at load time, so it is
// now exported properly.
export { createBOM, commonPrefix, prettyPrint, applyScriptChanges, enrichPackageJson, getCustomJsDependencies };
