...
class VPCSubnetFinder {
    ...
    async getRouteTables(vpcId) {
        const rts = await this.ec2.send(new DescribeRouteTablesCommand({
            Filters: [
                {
                    Name: 'vpc-id',
                    Values: [vpcId]
    ...
    async findDefaultVpc() {
        const vpcRes = await this.ec2.send(new DescribeVpcsCommand({
            Filters: [
                {
                    Name: 'isDefault',
                    Values: ['true']
    ...
    async getSubnets(vpcId) {
        const subRes = await this.ec2.send(new DescribeSubnetsCommand({
            Filters: [
                {
                    Name: 'vpc-id',
                    Values: [vpcId]
    ...
    isSubnetPublic(routeTables, subnetId) {
        ...
        if (subnetTable.length !== 1) {
            throw new Error(`Could not locate routing table for subnet: subnet id: ${subnetId}`);
        }
