...
import { CreateClusterCommand, DeregisterTaskDefinitionCommand, DescribeClustersCommand, DescribeTaskDefinitionCommand, ECSClient, InvalidParameterException, RegisterTaskDefinitionCommand, RunTaskCommand, StopTaskCommand } from '@aws-sdk/client-ecs';
import { GetRoleCommand, IAMClient } from '@aws-sdk/client-iam';
import { GetObjectCommand, NoSuchKey } from '@aws-sdk/client-s3';
import { CreateQueueCommand, DeleteQueueCommand, GetQueueAttributesCommand, ListQueuesCommand, SQSClient } from '@aws-sdk/client-sqs';
// Normal debugging for messages, summaries, and errors:
const debug = createDebug('commands:run-test');
// Verbose debugging for responses from AWS API calls, large objects etc:
const debugVerbose = createDebug('commands:run-test:v');
const debugErr = createDebug('commands:run-test:errors');
...
import rc from 'rc';
const defaultOptions = rc('artillery');
import EventEmitter from 'node:events';
import _ from 'lodash';
import moment from 'moment';
const pkg = require('artillery/package.json');
import dotenv from 'dotenv';
import { loadBuiltinPackage } from "../../../builtin-packages.js";
import awaitOnEE from "../../../util/await-on-ee.js";
import { setCloudwatchRetention } from "../../aws/aws-cloudwatch.js";
import getAccountId from "../../aws/aws-get-account-id.js";
import { parseResourceTags, toEcsTags, toTagMap } from "../../aws/resource-tags.js";
import * as awsUtil from "./aws-util.js";
import createS3Client from "./create-s3-client.js";
import { createTest } from "./create-test.js";
import { VPCSubnetFinder } from "./find-public-subnets.js";
import { SqsReporter } from "./sqs-reporter.js";
import { parseTags } from "./tags.js";
import { sleep, Timeout, timeStringToMs } from "./time.js";
import * as util from "./util.js";
import { getBucketName } from "./util.js";
export default runCluster;
let consoleReporter = {
    toggleSpinner: () => { }
};
import { ARTILLERY_CLUSTER_NAME, IMAGE_VERSION, LOGGROUP_NAME, LOGGROUP_RETENTION_DAYS, SQS_QUEUES_NAME_PREFIX, TASK_NAME, TEST_RUNS_MAX_TAGS, WAIT_TIMEOUT } from "./constants.js";
import { ClientServerVersionMismatchError, NoAvailableQueueError, TestNotFoundError } from "./errors.js";
let IS_FARGATE = false;
import { PutObjectCommand } from '@aws-sdk/client-s3';
import prepareTestExecutionPlan from "../../../util/prepare-test-execution-plan.js";
import awsGetDefaultRegion from "../../aws/aws-get-default-region.js";
import TEST_RUN_STATUS from "./test-run-status.js";
function setupConsoleReporter(quiet) {
    ...
    consoleReporter = global.artillery.__createReporter(reporterEvents, reporterOpts);
...
async function tryRunCluster(scriptPath, options, artilleryReporter) {
    ...
    if (context.tags.length > TEST_RUNS_MAX_TAGS) {
        console.error(chalk.red(`A maximum of ${TEST_RUNS_MAX_TAGS} tags is allowed per test run`));
        process.exit(1);
    }
    ...
    if (!context.namedTest) {
        ...
        const containerScriptPath = path.join(path.relative(contextPath, path.dirname(absoluteScriptPath)), path.basename(absoluteScriptPath));
        if (containerScriptPath.indexOf('..') !== -1) {
            artillery.log(chalk.red('Test script must reside inside the context dir. See Artillery docs for more details.'));
            process.exit(1);
        }
    ...
    if (options.launchConfig) {
        ...
        if (!launchConfig) {
            artillery.log(chalk.red("Launch config could not be parsed. Please check that it's valid JSON."));
            process.exit(1);
        }
    ...
    if (typeof options.launchType !== 'undefined') {
        if (options.launchType !== 'ecs:fargate' &&
            options.launchType !== 'ecs:ec2') {
            artillery.log('Invalid launch type - the value of --launch-type needs to be ecs:fargate or ecs:ec2');
            process.exit(1);
        }
    }
    if (typeof options.fargate !== 'undefined') {
        console.error('The --fargate flag is deprecated, use --launch-type ecs:fargate instead');
    }
    if (options.fargate && options.launchType) {
        console.error('Either --fargate or --launch-type flag should be set, not both');
        process.exit(1);
    }
    if (typeof options.fargate === 'undefined' &&
    ...
    context = Object.assign(context, {
        ...
        taskName: `${TASK_NAME}_${IS_FARGATE ? 'fargate' : ''}_${clusterName}_${IMAGE_VERSION.replace(/\./g, '-')}_${Math.floor(Math.random() * 1e6)}`,
    ...
    if (options.publicSubnetIds) {
        console.error(`${chalk.yellow('Warning')}: --public-subnet-ids will be deprecated. Use --subnet-ids instead.`);
        ...
        try {
            ...
            if (!clusterExists) {
                if (typeof context.cliOptions.cluster === 'undefined') {
                    ...
                    throw new Error(`Could not find cluster ${context.clusterName} in ${context.region}`);
                }
            }
            if (context.tags.length > 0) {
                logProgress(`Tags: ${context.tags
            ...
            logProgress(`Environment:
  Account:     ${context.accountId}
  Region:      ${context.region}
  Count:       ${context.count}
  Cluster:     ${context.clusterName}
  Launch type: ${context.cliOptions.launchType} ${context.isFargate && context.isCapacitySpot ? '(Spot)' : '(On-demand)'}
`, { showTimestamp: false });
            ...
            setCloudwatchRetention(`${LOGGROUP_NAME}/${context.clusterName}`, LOGGROUP_RETENTION_DAYS, context.region, {
                maxRetries: 10,
                waitPerRetry: 2 * 1000
            });
            if (context.status !== TEST_RUN_STATUS.EARLY_STOP &&
                context.status !== TEST_RUN_STATUS.TERMINATING) {
                logProgress(context.isFargate ? 'Waiting for Fargate...' : 'Waiting for ECS...');
            ...
            if (context.status !== TEST_RUN_STATUS.EARLY_STOP &&
                ...
                if (context.maxDurationMs && context.maxDurationMs > 0) {
                    logProgress(`Max duration for test run is set to: ${context.cliOptions.maxDuration}`);
                    const testDurationTimeout = new Timeout(context.maxDurationMs);
                    testDurationTimeout.start();
                    testDurationTimeout.on('timeout', async () => {
                        artillery.log(`Max duration of test run exceeded: ${context.cliOptions.maxDuration}\n`);
            ...
            const workerState = await awaitOnEE(artilleryReporter.reporterEvents, 'workersDone');
            debug(workerState);
            ...
            if (context.cliOptions.output) {
                const logfile = getLogFilename(context.cliOptions.output, defaultOptions.logFilenameFormat);
            ...
            if (err instanceof InvalidParameterException) {
                if (err.message
                    ...
                    artillery.log(chalk.yellow('The ECS cluster has no active EC2 instances'));
                }
                else {
                    artillery.log(err);
                }
            ...
            else if (err instanceof NoAvailableQueueError ||
...
async function cleanupResources(context) {
    try {
        ...
        if (context.adot?.SSMParameterPath) {
            await awsUtil.deleteParameter(context.adot.SSMParameterPath, context.region);
        }
...
function checkFargateResourceConfig(cpu, memory) {
    ...
    if (!FARGATE_VALID_CONFIGS[cpu]) {
        return new Error(`Unsupported cpu override for Fargate. Must be one of: ${Object.keys(FARGATE_VALID_CONFIGS).join(', ')}`);
    }
    if (FARGATE_VALID_CONFIGS[cpu].indexOf(memory) < 0) {
        return new Error(`Fargate memory override for cpu = ${cpu} must be one of: ${FARGATE_VALID_CONFIGS[cpu].join(', ')}`);
    }
    return null;
}
...
async function checkTargetCluster(context) {
    ...
    try {
        const response = await ecs.send(new DescribeClustersCommand({ clusters: [context.clusterName] }));
        debug(response);
        if ((response.clusters?.length ?? 0) === 0 ||
            (response.failures?.length ?? 0) > 0) {
            debugVerbose(response);
            return false;
        }
        else {
            const activeClusters = response.clusters.filter((c) => c.status === 'ACTIVE');
...
async function createADOTDefinitionIfNeeded(context) {
    const publishMetricsConfig = context.fullyResolvedConfig.plugins?.['publish-metrics'];
    if (!publishMetricsConfig) {
        debug('No publish-metrics plugin set, skipping ADOT configuration');
        return context;
    }
    const { getADOTRelevantReporterConfigs, resolveADOTConfigSettings } = await loadBuiltinPackage('artillery-plugin-publish-metrics');
    const adotRelevantConfigs = getADOTRelevantReporterConfigs(publishMetricsConfig);
    ...
    try {
        ...
        await awsUtil.putParameter(context.adot.SSMParameterPath, JSON.stringify(adotConfig), 'String', context.region);
...
async function ensureTaskExists(context) {
    ...
    const imageUrl = process.env.WORKER_IMAGE_URL ||
    ...
    try {
        ...
        if (process.env.ECR_IMAGE_VERSION) {
            debug('ECR_IMAGE_VERSION is set, but the task definition was already in place.');
        }
        return context;
    }
    catch (_err) {
        try {
            const response = await ecs.send(new RegisterTaskDefinitionCommand(taskDefinition));
...
async function checkCustomTaskRole(context) {
    ...
    const roleData = await iam.send(new GetRoleCommand({ RoleName: context.customTaskRoleName }));
...
async function gcQueues(context) {
    ...
    let data;
    try {
        data = await sqs.send(new ListQueuesCommand({
...
async function createQueue(context) {
    ...
    const queueName = `${SQS_QUEUES_NAME_PREFIX}_${context.testId.slice(0, 30)}.fifo`;
    const params = {
        QueueName: queueName,
        Attributes: {
            FifoQueue: 'true',
    ...
    while (waited < 120 * 1000) {
        try {
            const results = await sqs.send(new ListQueuesCommand({ QueueNamePrefix: queueName }));
...
async function generateTaskOverrides(context) {
    const cliArgs = ['run'].concat(context.cliOptions.environment
        ? ['--environment', context.cliOptions.environment]
        : [], context.cliOptions['scenarioName']
        ? ['--scenario-name', context.cliOptions['scenarioName']]
        : [], context.cliOptions.insecure ? ['-k'] : [], context.cliOptions.target ? ['-t', context.cliOptions.target] : [], context.cliOptions.overrides
    ...
    const s3path = `s3://${context.s3Bucket}/tests/${context.namedTest ? context.s3Prefix : context.testId}`;
    const adotOverride = [
        {
            name: 'adot-collector',
            environment: []
...
async function launchLeadTask(context) {
    const metadata = {
        ...
        tags: context.tags,
        secrets: JSON.stringify(Array.isArray(context.extraSecrets)
    ...
    const leaderParams = Object.assign({ count: 1 }, JSON.parse(JSON.stringify(context.defaultECSParams)));
    ...
    context.taskArns = context.taskArns.concat((runData.tasks ?? []).map((task) => task.taskArn));
...
async function ecsRunTask(context) {
    ...
    while (tasksRemaining > 0 &&
        ...
        const params = Object.assign({ count: launchCount }, JSON.parse(JSON.stringify(context.defaultECSParams)));
        ...
        try {
            ...
            if (runErr.name === 'ThrottlingException') {
    ...
    while (context.status !== TEST_RUN_STATUS.TERMINATING) {
        ...
        if (silentWaitTimeout.timedout()) {
            ...
            const statusSummary = _.map(statusCounts, (count, status) => {
                const displayStatus = status === 'RUNNING' ? 'ready' : status.toLowerCase();
...
async function waitForWorkerSync(context) {
    ...
    while (attempts < times) {
        try {
            const objects = await util.listAllObjectsWithPrefix(context.s3Bucket, prefix);
    ...
    return new Promise((resolve, _reject) => {
        ...
        r.on('workerDone', (body, attrs) => {
            if (process.env.LOG_WORKER_MESSAGES) {
                artillery.log(chalk.green(`[${attrs.workerId.StringValue} ${JSON.stringify(body, null, 4)}]`));
            }
        });
        r.on('workerError', (body, attrs) => {
            if (process.env.LOG_WORKER_MESSAGES) {
                artillery.log(chalk.red(`[${attrs.workerId.StringValue} ${JSON.stringify(body, null, 4)}]`));
            }
            if (body.exitCode !== 21) {
                artillery.log(chalk.yellow(`Worker exited with an error, worker ID = ${attrs.workerId.StringValue}`));
            }
        ...
        r.on('workerMessage', (body, attrs) => {
            if (process.env.LOG_WORKER_MESSAGES) {
                artillery.log(chalk.yellow(`[${attrs.workerId.StringValue}] ${body.msg} ${body.type}`));
            }
...
function getLogFilename(output, userDefaultFilenameFormat) {
    ...
    if (!isDir && output) {
        ...
        logfile = path.join(output, moment().format(userDefaultFilenameFormat || defaultFormat));
    }
    return logfile;
}
