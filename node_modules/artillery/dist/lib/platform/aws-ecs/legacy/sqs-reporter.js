...
const debug = createDebug('sqs-reporter');
const debugV = createDebug('sqs-reporter:v');
...
class SqsReporter extends EventEmitter {
    ...
    start() {
        ...
        this.intermediateReporterInterval = driftless.setDriftlessInterval(() => {
            ...
            const MAX_WAIT_FOR_PERIOD_MS = (Math.ceil(this.count / 10) * 2 + 20) * 1000;
            if (typeof earliestPeriodAvailable !== 'undefined' &&
                ...
                debug('have metrics from all workers for period or MAX_WAIT_FOR_PERIOD reached', earliestPeriodAvailable);
                ...
                const stats = global.artillery.__SSMS.mergeBuckets(this.metricsByPeriod[String(earliestPeriodAvailable)])[String(earliestPeriodAvailable)];
        ...
        this.workersDoneWatcher = driftless.setDriftlessInterval(() => {
            ...
            if (Object.keys(this.metricsByPeriod).length > 0) {
                debug('All workers done but still waiting on some intermediate reports');
                return;
            }
            ...
            if (process.env.DEBUG === 'sqs-reporter:v') {
                for (const [workerId, metrics] of Object.entries(this.metricsMessagesFromWorkers)) {
        ...
        this.ee.on('message', (body, attrs) => {
            ...
            if (body.event === 'phaseStarted') {
                if (typeof this.phaseStartedEventsSeen[body.phase.index] === 'undefined') {
            ...
            if (body.event === 'phaseCompleted') {
                if (typeof this.phaseCompletedEventsSeen[body.phase.index] ===
                    'undefined') {
            ...
            if (body.event === 'workerStats') {
                // v2 SSMS stats
                const workerStats = global.artillery.__SSMS.deserializeMetrics(body.stats);
                const period = workerStats.period;
                debug('processing workerStats event, worker:', workerId, 'period', period);
                debugV(workerStats);
                ...
                if (process.env.DEBUG === 'sqs-reporter:v') {
                    if (typeof this.metricsMessagesFromWorkers[workerId] === 'undefined') {
            ...
            handleMessage: async (message) => {
                ...
                if (body._overflowRef && this.s3 && this.s3Bucket) {
                    try {
                        ...
                        throw new Error(`Failed to fetch overflow message: ${body._overflowRef}`);
                    }
                }
    ...
    calculateSpread(stats) {
        const period = _.reduce(stats._requestTimestamps, (acc, ts) => {
