...
const _debug = createDebug('plugin:sqsReporter');
import { randomUUID as uuid } from 'node:crypto';
import { getAQS, sendMessage } from "./azure-aqs.js";
// SQS has 1MB message limit. Use 950KB threshold for safety margin.
const SQS_SIZE_LIMIT = 950 * 1024;
class ArtillerySQSPlugin {
    ...
    async sendSQS(body) {
        ...
        try {
            ...
            if (payloadSize > SQS_SIZE_LIMIT && this.s3 && this.s3Bucket) {
                ...
                _debug('Payload %d bytes exceeded limit, uploaded to S3: %s', payloadSize, s3Key);
            }
            const params = {
            ...
            // sendSQS is only called when this.sqs is set:
    ...
    async sendAQS(body) {
        this.unsent++;
        // sendAQS is only called when the Azure queue client is configured:
