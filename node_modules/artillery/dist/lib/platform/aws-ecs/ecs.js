...
import { S3_BUCKET_NAME_PREFIX } from "../aws/constants.js";
import { ensureParameterExists } from "./legacy/aws-util.js";
class PlatformECS {
    ...
    async init() {
        ...
        const bucketName = await ensureS3BucketExists(this.platformOpts.region, this.s3LifecycleConfigurationRules, false);
        global.artillery.s3BucketRegion = await getBucketRegion(bucketName);
        await this.createIAMResources(this.accountId, this.platformOpts.taskRoleName);
    }
    ...
    async createWorkerRole(accountId, taskRoleName) {
        ...
        const createRoleResp = await iam.send(new CreateRoleCommand({
        ...
        const createPolicyResp = await iam.send(new CreatePolicyCommand({
...
async function ensureSSMParametersExist(region) {
    await ensureParameterExists('/artilleryio/NPM_TOKEN', 'null', 'SecureString', region);
    await ensureParameterExists('/artilleryio/NPM_REGISTRY', 'null', 'String', region);
    await ensureParameterExists('/artilleryio/NPM_SCOPE', 'null', 'String', region);
    await ensureParameterExists('/artilleryio/ARTIFACTORY_AUTH', 'null', 'SecureString', region);
    await ensureParameterExists('/artilleryio/ARTIFACTORY_EMAIL', 'null', 'String', region);
    await ensureParameterExists('/artilleryio/NPMRC', 'null', 'SecureString', region);
    await ensureParameterExists('/artilleryio/NPM_SCOPE_REGISTRY', 'null', 'String', region);
}
export default PlatformECS;
