...
import { SQS_QUEUES_NAME_PREFIX } from "../aws/constants.js";
import { createTest } from "../aws-ecs/legacy/create-test.js";
import * as util from "../aws-ecs/legacy/util.js";
import { QueueConsumer } from "./aqs-queue-consumer.js";
const debug = createDebug('platform:azure-aci');
import fs from 'node:fs';
import path from 'node:path';
import dotenv from 'dotenv';
import { IMAGE_VERSION, WAIT_TIMEOUT } from "../aws-ecs/legacy/constants.js";
import { sleep, Timeout } from "../aws-ecs/legacy/time.js";
import { regionNames } from "./regions.js";
...
`);
        ...
        // Upload test bundle
        //
        this.blobServiceClient = new BlobServiceClient(`https://${this.storageAccount}.blob.core.windows.net`, credential);
        this.blobContainerClient = this.blobServiceClient.getContainerClient(this.blobContainerName);
        ...
        this.artilleryArgs.push(p.noPrefixPosix);
        const poolSize = typeof process.env.CONSUMER_POOL_SIZE !== 'undefined'
            ? parseInt(process.env.CONSUMER_POOL_SIZE, 10)
            : Math.max(Math.ceil(this.count / 25), 5);
        const consumer = new QueueConsumer({ poolSize }, {
            ...
            handleMessage: async (message) => {
                ...
                if (payload._overflowRef) {
                    try {
                        const blobClient = this.blobContainerClient.getBlockBlobClient(payload._overflowRef);
                        ...
                        throw new Error(`Failed to fetch worker message: ${payload._overflowRef}`);
                    }
                }
                ...
                if (payload.event === 'workerStats') {
                ...
                else if (payload.event === 'phaseStarted' ||
                    ...
                    if (payload.exitCode !== 21) {
                        this.events.emit(payload.event, workerId, {
                            id: workerId,
                            error: new Error(`A worker has exited with an error. Reason: ${payload.reason}`),
                            level: 'error',
                            aggregatable: false,
                            logs: payload.logs
                        });
    ...
    async startJob() {
        ...
        for (let i = 0; i < this.platformOpts.count - 1; i++) {
            ...
            if (i > 0 && i % 10 === 0) {
                const delayMs = Math.floor(Math.random() *
                    parseInt(process.env.AZURE_LAUNCH_STAGGER_SEC || '5', 10)) * 1000;
                await sleep(delayMs);
            }
        }
        let instancesCreated = false;
        console.log('Waiting for Azure ACI to create container instances...');
        const containerInstanceClient = new ContainerInstanceManagementClient(new DefaultAzureCredential(), this.azureSubscriptionId);
        const provisioningWaitTimeout = new Timeout(WAIT_TIMEOUT * 1000).start();
        let containerGroupsInTestRun = [];
        while (true) {
            const containerGroupListResult = containerInstanceClient.containerGroups.listByResourceGroup(this.resourceGroupName);
        ...
        if (instancesCreated) {
            console.log('Container instances have been created. Waiting for workers to start...');
            ...
            console.log('https://portal.azure.com/#view/HubsExtension/BrowseResource/resourceType/Microsoft.ContainerInstance%2FcontainerGroups');
    ...
    async shutdown() {
        ...
        if (process.env.RETAIN_CONTAINER_INSTANCES !== 'true') {
            const containerInstanceClient = new ContainerInstanceManagementClient(credential, this.azureSubscriptionId);
            const containerGroupListResult = containerInstanceClient.containerGroups.listByResourceGroup(this.resourceGroupName);
            for await (const containerGroup of containerGroupListResult) {
                // NOTE: pre-existing behavior: a group with no name throws.
                if (containerGroup.name.indexOf(this.testRunId) > 0) {
                    try {
                        await containerInstanceClient.containerGroups.beginDeleteAndWait(this.resourceGroupName, containerGroup.name);
                    }
                    catch (err) {
                        console.log(err);
                    }
    ...
    async runWorker(workerId, opts = { isLeader: false }) {
        ...
        const containerImageURL = process.env.WORKER_IMAGE_URL ||
            `public.ecr.aws/d8a4z9o5/artillery-worker:${imageVersion}-${defaultArchitecture}`;
        const client = new ContainerInstanceManagementClient(credential, this.azureSubscriptionId);
        ...
        try {
            const containerInstance = await client.containerGroups.beginCreateOrUpdate(this.resourceGroupName, containerGroupName, containerGroup);
    ...
    async checkLicense() {
        ...
        const baseUrl = process.env.ARTILLERY_CLOUD_ENDPOINT || 'https://app.artillery.io';
        ...
        try {
            ...
            const activeMembership = body.memberships.find((membership) => membership.id === activeOrg);
            if (!activeMembership) {
                return false;
            }
...
export default PlatformAzureACI;
