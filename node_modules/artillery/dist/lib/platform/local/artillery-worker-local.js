...
import STATES from "../worker-states.js";
...
// the sibling is worker.ts (worker threads strip types natively).
const workerModulePath = path.join(import.meta.dirname, import.meta.filename.endsWith('.ts') ? 'worker.ts' : 'worker.js');
...
class ArtilleryWorker {
    ...
    async init(_opts) {
        ...
        this.worker = new Worker(workerModulePath, {
            env: workerEnv
        });
