...
const debug = createDebug('util:aws:ensureS3BucketExists');
import { CreateBucketCommand, NoSuchBucket, PutBucketLifecycleConfigurationCommand, S3Client } from '@aws-sdk/client-s3';
import createS3Client from "../aws-ecs/legacy/create-s3-client.js";
import getAWSAccountId from "./aws-get-account-id.js";
import { getBucketRegion } from "./aws-get-bucket-region.js";
import { S3_BUCKET_NAME_PREFIX } from "./constants.js";
const setBucketLifecyclePolicy = async (bucketName, lifecycleConfigurationRules, region) => {
...
export default async function ensureS3BucketExists(region, lifecycleConfigurationRules = [], withRegionSpecificName = false) {
    ...
    if (lifecycleConfigurationRules.length > 0) {
        await setBucketLifecyclePolicy(bucketName, lifecycleConfigurationRules, location);
    }
    debug(bucketName);
    return bucketName;
}
