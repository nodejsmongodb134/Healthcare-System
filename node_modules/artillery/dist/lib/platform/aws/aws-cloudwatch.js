...
const debug = createDebug('artillery:aws-cloudwatch');
const allowedRetentionDays = [
    1, 3, 5, 7, 14, 30, 60, 90, 120, 150, 180, 365, 400, 545, 731, 1096, 1827,
    2192, 2557, 2922, 3288, 3653
];
async function _putCloudwatchRetentionPolicy(logGroupName, retentionInDays, region) {
    ...
    return cloudwatchlogs.send(new PutRetentionPolicyCommand(putRetentionPolicyParams));
}
function setCloudwatchRetention(logGroupName, 
...
retentionInDays, region, options = { maxRetries: 5, waitPerRetry: 1000 }) {
    if (!allowedRetentionDays.includes(retentionInDays)) {
        console.log(`WARNING: Skipping setting CloudWatch retention, as invalid value specified: ${retentionInDays}. Allowed values are: ${allowedRetentionDays.join(', ')}`);
        return;
    }
    const interval = setInterval(async (opts) => {
        debug(`Trying to set CloudWatch Log group ${logGroupName} retention policy to ${retentionInDays} days`);
        opts.incr = (opts.incr || 0) + 1;
        try {
            const _res = await _putCloudwatchRetentionPolicy(logGroupName, retentionInDays, region);
            debug(`Successfully set CloudWatch Logs retention policy to ${retentionInDays} days`);
            clearInterval(interval);
        }
        catch (err) {
            const error = err;
            const resumeTestMessage = 'The test will continue without setting the retention policy.';
    ...
    }, options.waitPerRetry, options);
    return interval;
}
export { setCloudwatchRetention };
