...
function parseResourceTags(input) {
    if (!input) {
        return [];
    }
    const tags = [];
    ...
    for (const entry of entries) {
        ...
        if (key.length === 0) {
            ...
            errors.push(`"${entry}" - tag key exceeds ${TAG_KEY_MAX_LENGTH} characters`);
        }
            ...
            errors.push(`"${entry}" - tag value exceeds ${TAG_VALUE_MAX_LENGTH} characters`);
        }
