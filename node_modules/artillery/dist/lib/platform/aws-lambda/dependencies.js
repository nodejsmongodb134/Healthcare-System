...
import { createBOM, enrichPackageJson } from "../aws-ecs/legacy/bom.js";
import createS3Client from "../aws-ecs/legacy/create-s3-client.js";
const _createLambdaBom = async (absoluteScriptPath, absoluteConfigPath, flags) => {
    const createBomOpts = {};
...
async function _syncS3(bomManifest, testRunId, bucketName) {
    ...
    for (const file of bomManifest.files) {
        await _uploadFileToS3(file, testRunId, bucketName, bomManifest.moduleVersions);
        fileCount++;
    }
    ...
    await plainS3.send(new PutObjectCommand({
        Bucket: bucketName,
        Key: key,
        // TODO: stream, not readFileSync
...
const createAndUploadTestDependencies = async (bucketName, testRunId, absoluteScriptPath, absoluteConfigPath, flags) => {
    const bom = await _createLambdaBom(absoluteScriptPath, absoluteConfigPath, flags);
