...
import { CreateFunctionCommand, DeleteFunctionCommand, GetFunctionConfigurationCommand, InvokeCommand, LambdaClient, ResourceConflictException, ResourceNotFoundException, TagResourceCommand } from '@aws-sdk/client-lambda';
import { PutObjectCommand } from '@aws-sdk/client-s3';
import { DeleteQueueCommand, SQSClient } from '@aws-sdk/client-sqs';
import _ from 'lodash';
import { QueueConsumer } from "../../queue-consumer/index.js";
import * as telemetry from "../../telemetry.js";
import sleep from "../../util/sleep.js";
import createSQSQueue from "../aws/aws-create-sqs-queue.js";
import ensureS3BucketExists from "../aws/aws-ensure-s3-bucket-exists.js";
import getAccountId from "../aws/aws-get-account-id.js";
import { getBucketRegion } from "../aws/aws-get-bucket-region.js";
import awsGetDefaultRegion from "../aws/aws-get-default-region.js";
import { SQS_QUEUES_NAME_PREFIX } from "../aws/constants.js";
import { parseResourceTags, toTagMap } from "../aws/resource-tags.js";
import createS3Client from "../aws-ecs/legacy/create-s3-client.js";
import { createAndUploadTestDependencies } from "./dependencies.js";
...
class PlatformLambda {
    constructor(script, payload, opts, platformOpts) {
        ...
        this.resourceTags = parseResourceTags(this.platformOpts.cliArgs['aws-tags']);
    ...
    async init() {
        ...
        const bucketName = await ensureS3BucketExists(this.region, this.s3LifecycleConfigurationRules, true);
        this.bucketName = bucketName;
        global.artillery.s3BucketRegion = await getBucketRegion(bucketName);
        const { bom, s3Path } = await createAndUploadTestDependencies(this.bucketName, this.testRunId, this.opts.absoluteScriptPath, this.opts.absoluteConfigPath, this.platformOpts.cliArgs);
        ...
        const sqsQueueUrl = await createSQSQueue(this.region, queueName, toTagMap(this.resourceTags));
        ...
        consumer.create({
            ...
            handleMessage: async (message) => {
                ...
                else if (body.event === 'phaseStarted' ||
        ...
        global.artillery.ext({
            ...
            method: async (event) => {
                ...
                if (event.flags && event.flags.platform === 'aws:lambda') {
                    ...
                    const total = ((price * this.memorySize) / 1024) *
    ...
    async runWorker(workerId) {
        ...
        while (waited < timeout) {
            try {
                state = (await lambda.send(new GetFunctionConfigurationCommand({
    ...
    async createLambdaRole() {
        ...
        const iamRes = await iam.send(new CreatePolicyCommand({
            PolicyDocument: `{
        "Version": "2012-10-17",
        "Statement": [
          {
    ...
    async createOrUpdateLambdaFunctionIfNeeded() {
        ...
        if (existingLambdaConfig) {
            debug('Lambda function with this configuration already exists. Using existing function.');
        ...
        try {
            ...
            if (err instanceof ResourceConflictException) {
                debug('Lambda function with this configuration already exists. Using existing function.');
                await this.tagExistingFunction(`${this.arnPrefix}:lambda:${this.region}:${this.accountId}:function:${this.functionName}`);
                return;
            }
    ...
    async tagExistingFunction(functionArn) {
        ...
        try {
            ...
            artillery.log(`WARNING: could not apply resource tags to Lambda function: ${err.message}`);
            debug(err);
        }
    }
    async getLambdaFunctionConfiguration() {
        ...
        try {
            const res = await lambda.send(new GetFunctionConfigurationCommand({
                FunctionName: this.functionName
            }));
            return res;
        }
    ...
    createFunctionNameWithHash(_lambdaConfig) {
        ...
        const configHash = crypto
            ...
            .digest('hex');
        let name = `artilleryio-v${this.currentVersion.replace(/\./g, '-')}-${this.architecture}-${configHash}`;
        if (name.length > 64) {
