...
import { loadPlugins, loadPluginsConfig } from "./load-plugins.js";
const debug = createDebug('core');
import { promisify as p } from 'node:util';
import _ from 'lodash';
import PlatformLambda from "./platform/aws-lambda/index.js";
import PlatformAzureACI from "./platform/az/aci.js";
import PlatformLocal from "./platform/local/index.js";
...
class Launcher {
    ...
    async initWorkerEvents(workerEvents) {
        ...
        workerEvents.on('phaseCompleted', (_workerId, message) => {
            if (typeof this.phaseCompletedEventsSeen[message.phase.index] ===
                'undefined') {
        ...
        workerEvents.on('done', async (workerId, message) => {
            this.exitedWorkersCount++;
            this.finalReportsByWorker[workerId] = SSMS.deserializeMetrics(message.report);
        });
    ...
    async initPlugins() {
        const plugins = await loadPlugins(this.script.config.plugins, this.script, this.opts);
        //
        // init plugins
        //
                ...
                if (result.version === 3) {
                    ...
                    if (result.version === 1) {
                        ...
                        if (result.PluginExport.LEGACY_METRICS_FORMAT === false) {
                            result.plugin = new result.PluginExport.Plugin(dummyScript, this.pluginEvents, this.opts);
                        }
                        else {
    ...
    async handleAllWorkersFinished() {
        const allWorkersDone = this.exitedWorkersCount === this.platform.getDesiredWorkerCount();
        if (allWorkersDone) {
    ...
    async flushIntermediateMetrics(flushAll = false) {
        ...
        const MAX_WAIT_FOR_PERIOD_MS = (Math.ceil(this.platform.getDesiredWorkerCount() / 10) * 3 + 30) * 1000;
        debug({
            now: Date.now(),
        ...
        const allWorkersReportedForPeriod = this.metricsByPeriod[earliestPeriodAvailable]?.length ===
        ...
        else if (typeof earliestPeriodAvailable !== 'undefined' &&
    ...
    emitIntermediatesForPeriod(period) {
        debug('Report @', new Date(Number(period)), 'made up of items:', this.metricsByPeriod[String(period)].length);
