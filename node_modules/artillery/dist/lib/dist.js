...
import { isIdlePhase } from "./core/index.js";
...
function divideWork(script, numWorkers) {
    ...
    const result = workerScripts.filter((workerScript) => !workerScript.config.phases.every(isIdlePhase));
    ...
    for (let i = 0; i < result.length; i++) {
        ...
        if (hasPayload) {
            ...
            for (let payloadIdx = 0; payloadIdx < payload.length; payloadIdx++) {
                ...
                result[i].config.payload[payloadIdx].data = scriptPayloadData.filter((_, index) => index % result.length === idxToMatch);
            }
        }
    }
    return result;
...
function handleArrivalRatePhase(phase, numWorkers, workerScripts) {
    ...
    const activeWorkers = rates.reduce((acc, rate) => acc + (rate > 0 ? 1 : 0), 0);
