...
// Used for: loading user-provided .js scripts and esbuild-generated
// CJS bundles synchronously, and lazy-loading esbuild-wasm (removed
// from the Lambda bundle to keep package size down).
const require = createRequire(import.meta.url);
import _ from 'lodash';
import { addDefaultPlugins, addOverrides, addVariables, checkConfig, parseScript, readScript, resolveConfigPath, resolveConfigTemplates } from "../util.js";
...
async function prepareTestExecutionPlan(inputFiles, flags, _args) {
    ...
    for (const fn of inputFiles) {
        ...
        if (fn2.endsWith('.yml') ||
            ...
            if (fn2.endsWith('.js')) {
                ...
                const outputPath = path.join(path.dirname(absoluteFn), `dist/${path.basename(fn)}.js`);
                ...
                console.log('Only JSON (.json), YAML (.yml/.yaml) and TypeScript (.ts) files are supported');
    ...
    const hasOriginalTarget = typeof script1.config.target !== 'undefined' ||
        typeof script1.config.environments?.[flags.environment]?.target !== 'undefined';
    ...
    script4._configPath, script4._scriptPath);
...
async function readPayload(script) {
    ...
    for (const payloadSpec of script.config.payload) {
        ...
        const csvOpts = Object.assign({
            skip_empty_lines: typeof payloadSpec.skipEmptyLines === 'undefined'
                ? true
...
function replaceProcessorIfTypescript(script, scriptPath) {
    ...
    const actualProcessorPath = path.resolve(path.dirname(scriptPath), relativeProcessorPath);
    ...
    const newProcessorPath = path.join(processorDir, `dist/${processorFileName}.js`);
    ...
    console.log(`Bundled Typescript file into JS. New processor path: ${newProcessorPath}`);
    script.config.processor = newProcessorPath;
    return script;
}
export default prepareTestExecutionPlan;
