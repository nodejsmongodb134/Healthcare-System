...
import { getCustomJsDependencies } from "../platform/aws-ecs/legacy/bom.js";
import { parseScript, readScript } from "../util.js";
import BUILTIN_PLUGINS from "./built-in-plugins.js";
...
async function createBOM(absoluteScriptPath, extraFiles, opts, callback) {
    A.waterfall([
        ...
        return callback(null, {
            ...
            modules: _.uniq(context.npmModules).filter((m) => m !== 'artillery' &&
                m !== 'playwright' &&
...
function getCustomEngines(context, next) {
    ...
    const engineNames = _.uniq(Object.keys(_.get(context, 'opts.scriptData.config.engines', {})));
        ...
        if (obj.payload) {
            if (_.isArray(obj.payload)) {
                obj.payload.forEach((payloadSpec) => {
                    result.push(path.resolve(path.dirname(context.opts.absoluteScriptPath), payloadSpec.path));
                });
            }
            else if (_.isObject(obj.payload)) {
                // NOTE: isObject returns true for arrays, so this branch must
                // come second.
                result.push(path.resolve(path.dirname(context.opts.absoluteScriptPath), obj.payload.path));
            }
        }
        return result;
    }
...
function getExtraFiles(context, next) {
    if (context.opts.scriptData.config?.includeFiles) {
        const absPaths = _.map(context.opts.scriptData.config.includeFiles, (p) => {
            const includePath = path.resolve(path.dirname(context.opts.absoluteScriptPath), p);
...
export { createBOM, commonPrefix, prettyPrint };
