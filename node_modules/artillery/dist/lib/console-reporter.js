...
class ConsoleReporter {
    ...
    phaseStarted(phase) {
        ...
        const durationString = Number.isInteger(_.toNumber(phaseDuration))
            ? `${phaseDuration}s`
            : `${phaseDuration}`;
        artillery.log(`Phase started: ${chalk.green(phase.name ? phase.name : 'unnamed')} (index: ${phase.index}, duration: ${durationString}) ${formatTimestamp(new Date())}\n`);
        return this;
    }
    phaseCompleted(phase) {
        ...
        const durationString = Number.isInteger(_.toNumber(phaseDuration))
            ? `${phaseDuration}s`
            : `${phaseDuration}`;
        artillery.log(`Phase completed: ${chalk.green(phase.name ? phase.name : 'unnamed')} (index: ${phase.index}, duration: ${durationString}) ${formatTimestamp(new Date())}\n`);
        return this;
    }
    stats(data) {
        if (this.quiet) {
        ...
        if (typeof data.report === 'function') {
            ...
            artillery.log(`Elapsed time: ${util.formatDuration(Date.now() - this.startTime)}`);
        ...
        if (this.startTime !== null) {
            artillery.log(`All VUs finished. Total time: ${util.formatDuration(Date.now() - this.startTime)}\n`);
        }
    ...
    printReport(report, opts) {
        ...
        if (this.outputFormat === 'new') {
            ...
            const sortedByLen = _(Object.keys(report.summaries)
...
function padded(str1, str2) {
    ...
    // We need at least 50
    const columnsAvailable = Math.max(process.stdout?.columns || defaultWidth, 50);
    // But no more than 79:
