...
class SSMS extends EventEmitter {
    ...
    static legacyReport(pd) {
        ...
        const result = {
            ...
            requestsCompleted: counters['http.responses'] ||
        ...
        if (histograms &&
            ...
            result.scenarioDuration = summarizeHistogram(histograms['vusers.session_length']);
        }
        // scenarioCounts
        const names = Object.keys(counters).filter((k) => k.startsWith('vusers.created_by_name.'));
        ...
        // HTTP codes
        const codeNames = Object.keys(counters).filter((k) => k.match(/^(http|socketio)\.codes.*/));
        for (const n of codeNames) {
            const code = parseInt(n.split('.codes.')[1], 10);
            result.codes[code] = counters[n];
        }
        // errors
        const errNames = Object.keys(counters).filter((k) => k.startsWith('errors.'));
