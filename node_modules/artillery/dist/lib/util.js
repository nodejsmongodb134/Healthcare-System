...
import { engine_util as engineUtil } from "./commons/index.js";
const renderVariables = engineUtil._renderVariables;
const template = engineUtil.template;
import { runner as __runner } from "./core/index.js";
const { contextFuncs } = __runner;
import { promisify as p } from 'node:util';
export { readScript, parseScript, addOverrides, addVariables, addDefaultPlugins, resolveConfigPath, resolveConfigTemplates, checkConfig, renderVariables, template, formatDuration, padded, rainbow };
...
async function addOverrides(script, flags) {
    ...
    const result = _.mergeWith(script, o, function customizer(_objVal, srcVal, _k, _obj, _src, _stack) {
        if (_.isArray(srcVal)) {
            return srcVal;
        }
        else {
...
async function checkMergedConfig(script, scriptPath, flags) {
    if (flags.environment) {
        ...
        if (script.config.environments?.[flags.environment]) {
            ...
            console.log(`WARNING: environment ${flags.environment} is set but is not defined in the script`);
        }
    }
    ...
    if (_.get(script, 'config.payload')) {
        ...
        if (_.isArray(script.config.payload)) {
            ...
            const payload = script.config.payload;
    ...
    _.forEach(script.config.payload, (payloadSpec) => {
        const resolvedPathToPayload = path.resolve(path.dirname(absoluteScriptPath), payloadSpec.path);
...
async function resolveConfigPath(script, flags, scriptPath) {
    ...
    const processorPath = path.resolve(path.dirname(absoluteConfigPath), script.config.processor);
    const stats = fs.statSync(processorPath, { throwIfNoEntry: false });
    if (typeof stats === 'undefined') {
        // No file at that path - backwards compatibility mode:
        console.log('WARNING - config.processor is now resolved relative to the config file');
